...
Docker (не systemd). `./deploy.sh` собирает `linux/amd64`-образ локально, шлёт по SSH
(`docker save | ssh | docker load`, без реестра) и `docker compose up -d` на узле (RU1
RAM-голодный — Go/реестр на узле не нужны). Подробности — `README.md` / `docs/subgen.md`.
Legacy systemd-юнит (`systemd/`) остановлен/выключен при первом docker-деплое. SIGHUP-релоада
нет — конфиг течёт снизу вверх из стора на каждый запрос. **Миграции БД — только руками**
(схема `CREATE … IF NOT EXISTS`).
...
- Роутинг — `gorilla/mux`: метод и path-параметры (`/sub/{token}`) задаёт роутер
...
migrations/init.sql            — миграции БД
```
...
адаптер, один метод — один файл); репозитории — `internal/repository/{users,nodes,routing}`
(один метод — один файл); сервисы — `internal/service/{fleet,ruleset,provisioning}`
...
атомарно (`SaveMihomoConfig`: группы+элементы+правила+провайдеры+base); таблицы
mihomo-конфига — с префиксом `mihomo_` (FK на `node_inbounds` RESTRICT). Ссылки на
...
не пишем; структурные изменения мигрируются руками).

...
- **Валидация значений — в коде, не в схеме.** В OpenAPI кладём только форму контракта —
  ...
  **не** кладём: ogen на них отдаёт общий невнятный `400 "Некорректный запрос"`. Вместо
  этого валидируем в коде — доменное правило sentinel'ом в сервисе, presence/`id>0` —
  гардом в хендлере с локальной текст-константой и типизированным 4xx. См.

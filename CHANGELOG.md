...
(`subLinker`/`configResolver`/`creator`/…); правило закреплено в `AGENTS.md`. `make lint`
теперь гоняет golangci-lint в Docker тем же pinned-образом `golangci/golangci-lint`
(linux/amd64), что и GitHub Actions, — локальный линт больше не расходится с CI (ловит
в т.ч. `wsl_v5`, который host-сборка под darwin/arm64 пропускала).

...
- **`Client` (SDK)** — типизированный HTTP-SDK к запущенному серверу. Ядро
  ...
  продакшн при этом не трогается). На каждый эндпоинт — типизированный метод; мутации
  декодятся в `Result{OK, Msg, Err}` (тот самый envelope `{ok,msg|err}`), read-ручки — в
  `User`/`Node`/`Config`/`Schema`. Для угловых случаев есть «сырые» формы (`PostRaw`,
  `LoginRaw`, `SaveConfigRaw`, `Get`/`GetURL`), отдающие статус/тело/заголовки.

...
- **Валидация — в сервисном слое**, sentinel-ошибками в `entity`; хендлеры тонкие и мапят
  ...
  `entity.ErrValidation*` — имя/хост/URL/base-path/инбаунды) плюс save/delete и FK-блоком
  (структурный `entity.InboundsBlockedError`, форматируемый на презентации). Часть валидаций
  уже была в сервисе/домене и оставлена: `validateName`, `ErrNoConnectionSelected`,
  `PolicyRef.Valid()`.
...
- Где старое поведение уже корректно отвергало вход без openapi-гарда — оставлено: пустые
  ...
  `PolicyRef.kind` → `validateRef`. Пустой URL provider-check — тонкий presence-гард в хендлере.
...
- Node-валидация переехала из `web` (handler-layer) в `internal/service/nodes`; `web` держит
  только форматтер сообщения FK-блока (человеко-текст — на презентации).

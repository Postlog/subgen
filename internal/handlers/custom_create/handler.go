...
const (
	msgConfigExists = "У пользователя уже есть кастомный конфиг"
...
func (h *Handler) CustomCreate(ctx context.Context, req *oas.CustomCreateReq) (oas.CustomCreateRes, error) {
	if req.UserId < 1 {
		slog.Warn("handler custom_create: invalid user id", "userId", req.UserId)
		return &oas.CustomCreateBadRequest{ErrMessage: msgInvalidID}, nil
	}

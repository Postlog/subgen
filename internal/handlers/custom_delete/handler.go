...
const (
	msgConfigMissing = "У пользователя нет кастомного конфига"
...
func (h *Handler) CustomDelete(ctx context.Context, req *oas.CustomDeleteReq) (oas.CustomDeleteRes, error) {
	if req.UserId < 1 {
		slog.Warn("handler custom_delete: invalid user id", "userId", req.UserId)
		return &oas.CustomDeleteBadRequest{ErrMessage: msgInvalidID}, nil
	}
